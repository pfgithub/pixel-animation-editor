...
Array.from(document.getElementsByTagName("button")).forEach(button => {
    ...
    button.setAttribute("title", hotkey ? `Press ${hotkey} to click` : `No hotkey set for this button.`);
...
function download(filename, text, dnlshow) {
    if (dnlshow) {
        ...
        const textarea = (myWindow.document.getElementById("hello"));
...
function play(frame) {
    ...
    renderFrame(true);
    if (frames[frame + 1]) {
        setTimeout(() => play(frame + 1), +getElement("playSpeed", HTMLInputElement).value);
    }
    else {
